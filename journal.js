...
function renderFavMovies() {
  ...
  favMovies.forEach((movie)=>{
    ...
    movieCard.innerHTML = `
    <img class="w-full h-[300px] object-cover rounded" src="https://image.tmdb.org/t/p/w500/${movie.poster_path}" 
    alt="${movie.title}"/>
    <h2 class="mt-2 text-lg font-semibold">${movie.title.charAt(0).toUpperCase()+ movie.title.slice(1)}</h2>
    <p class="text-gray-500 text-md">${movie.overview.substring(
        0,
        100
      )}..</p>
...
function addToFavorite(movie){
  ...
  alert(
    ...
    `${movie.title} is already exists to Favorite list`
  );
